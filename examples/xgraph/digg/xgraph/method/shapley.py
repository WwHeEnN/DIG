...
def GnnNetsGC2valueFunc(gnnNets, target_class):
    def value_func(batch):
        with torch.no_grad():
            ...
            probs = F.softmax(logits, dim=-1) # bw commented
            try: # bw 
                score = probs[:, target_class] # bw commented
            except: # bw 
                print(logits[-1].sum()) # bw 
            score=logits[-1].sum() # bw 
        return score
    return value_func
...
def marginal_contribution(data: Data, exclude_mask: np.array, include_mask: np.array,
    ...
    # print(marginal_contribution_list) # bw
    try: # bw 
        marginal_contributions = torch.cat(marginal_contribution_list, dim=0) # bw commented
    except: # bw
        # marginal_contributions = torch.cat(marginal_contribution_list, dim=0) # bw commented 
        # marginal_contributions = torch.cat(marginal_contribution_list.reshape(1), dim=0) # bw 
        marginal_contributions = torch.tensor(marginal_contribution_list) # bw
        # marginal_contributions = torch.cat(marginal_contribution_list) # bw
